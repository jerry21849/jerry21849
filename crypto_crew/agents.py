"""Agent definitions — 10 CrewAI agents with role-specific tools."""
...
def tool_run_backtest(strategy: str = "rsi", candles_json: str = "", params_json: str = "{}") -> str:
    """Run a backtest. strategy: 'rsi', 'ma_cross', 'bollinger'. candles_json from get_historical_data."""
    import json
...
def tool_get_portfolio() -> str:
    """Get current paper trading portfolio state (cash, positions, history)."""
    return paper_trade.get_paper_portfolio().model_dump_json()


@tool("execute_paper_trade")
def tool_execute_trade(action: str, symbol: str, amount_usd: float, price: float) -> str:
    """Execute a paper trade. action: 'buy' or 'sell'. Updates portfolio state."""
    return paper_trade.execute_paper_trade(action, symbol, amount_usd, price).model_dump_json()


@tool("compute_risk_metrics")
def tool_compute_risk(candles_json: str, portfolio_value: float = 10000.0, risk_profile: str = "conservative") -> str:
    """Compute VaR, ATR, volatility, position sizing from candle data."""
    import json
    candles = json.loads(candles_json) if candles_json else []
    return risk.compute_risk_metrics(candles, portfolio_value=portfolio_value, risk_profile=risk_profile).model_dump_json()
...
PREDICTION_AGENT = Agent(
    ...
    tools=[tool_get_crypto_price, tool_get_historical_data, tool_get_news,
           tool_calc_technical, tool_get_onchain, tool_get_sentiment],
    verbose=True,
)

BACKTEST_AGENT = Agent(
    role="Backtesting Agent — 量化回測工程師",
    goal="使用 RSI、MA 交叉、布林帶策略執行回測，輸出總報酬、Sharpe、Max Drawdown、勝率、權益曲線。",
...
PAPER_AGENT = Agent(
    ...
    goal="維護虛擬倉位，記錄交易日誌，給出買賣建議。0.1% 手續費，支援滑點模擬。",
